...
#define TWEB_VERSION 			'TWeb V2.1.000'
#define TWEB_PATH 				'lib/tweb/'
#define CRLF 					Chr(13)+Chr(10)

#xcommand ? [<explist,...>] => AP_Echo( '<br>' [,<explist>] )
#xcommand ?? <cText> => AP_Echo( <cText> )
#xcommand BLOCKS ADDITIVE <v>[ PARAMS [<v1>] [,<vn>] ] => ;
	#pragma __cstream |<v>+= mh_InlinePrg( mh_ReplaceBlocks( %s, "{{", "}}" [,<(v1)>][+","+<(vn)>] [, @<v1>][, @<vn>] ) )
...
function TWebHtmlInline( cFile, aParam )
	...
	cTxt := mh_ReplaceBlocks( cTxt, '<$', "$>", nil, aParam )			
