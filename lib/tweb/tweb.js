...
function TWebBrowse( cId, aData, lInit ) {
	...
	*/
	...
	this.SelectIndex = function() {		
		...
		var cName = cId + '_btSelectItem' 
		
		$('#' + cId + ' input[name="' + cName + '"]:checked').each(function () {
	...
	*/
	...
	/*	Difference with AddRow:
		InsRow only insert item to last 	
	*/
	
	this.InsRow = function( oItem ) {	

		var aData = oBrowse.bootstrapTable('getData')
		var nIndex		= aData.length 		
...
function TWebBrwSelect( aHeaders, aData, fCallback, cTitle, oOptions, aFormatter ) {

	cTitle 		= (typeof cTitle ) == 'string' ? cTitle : '<i class="fas fa-search"></i> Buscar' ;
	//aFormatter  = (typeof aFormatter == 'object' ) ? aFormatter : [];
	...
	if ( oOptions[ 'select' ] ) {
		...
		for (var key in aHeaders ) {							
			cMsg += '<th data-field="' + key + '" '			
			
			if ( typeof aFormatter == 'object' ) {
			
				if (key in aFormatter) {						
					cMsg += 'data-formatter="' + aFormatter[ key] + '" '
				}
			}
			
				
			cMsg += '>' + aHeaders[key] + '</th>' 
		}
