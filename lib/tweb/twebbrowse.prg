...
METHOD Activate() CLASS TWebBrowse
	...
	local u 
	local lCombo, lLogic, lDate, cSelectName
	local lSelect 			:= .F. 
	...
	*/	
	...
	//	Pagination 
		...
		*/
		...
		IF ::lMultiSelect .OR. ::lSingleSelect .OR. lSelect 
			cSelectName := ::cId + '_btSelectItem'
			cHtml += 'data-select-item-name="' + cSelectName + '" '
	...
	ENDTEXT														
								IF ::lMultiSelect .OR. ::lSingleSelect .OR. lSelect 																
									...
									cHtml += '<th data-field="' + cSelectName + '" data-checkbox="true" data-radio="' + If( ::lRadio, 'true', 'false' ) + '" name="' + cSelectName + '"></th>'
								...
								FOR n := 1 TO Len( ::hCols )
									...
									if ! hDef[ 'hidden'] 									
										...
										cHtml += 'data-width="' + mh_valtochar(hDef[ 'width' ]) + '" '
...
METHOD GetErrorString() CLASS TBrwDataset 
	...
	for nI := 1 to len( ::aError )
		...
		cError += mh_valtochar(::aError[nI]) + '<br>' 
...
static function DoBreak(oError) 
	...
	cError += '<b>System:</b> ' + oError:subsystem + '/' + mh_valtochar( oError:subcode) + '<br>'
	cError += '<b>Args:</b><br>'
	
	for n = 1 to len(oError:args)
		cError += Replicate( '&nbsp;', 5 ) + ltrim(str(n)) + ' => ' + mh_valtochar( oError:args[n] ) + '<br>'
	next
