...
function main()
	...
	DEFINE FORM o
		...
		HTML o INLINE '<h3>Basic Browse - PreEdit/PostEdit</h3><hr>'
	...
	INIT FORM o 
		...
		DEFINE BROWSE oBrw ID 'ringo' MULTISELECT CLICKSELECT HEIGHT 400 ;
			EDIT UNIQUEID 'id' TITLE '<i class="fas fa-recycle"></i> My ABM...' PREEDIT 'TestPreEdit' POSTEDIT 'TestPostEdit';
			TOOLBAR "bar" ;
			SEARCH  ;
			OF o
			...
			<script>
				...
				function TestPreEdit( oItemEdit, oItemBrw ) {
					...
					if ( nQty > 100 ) {
						...
						MsgError( 'Max. 100', 'Check Console'  )
						return false 
				...
				function TestPostEdit( oItemEdit, lUpdated ) {
				
					console.log( 'PostEdit oItemEdit', oItemEdit )
					console.log( 'PostEdit lUpdate', lUpdated )
					
					if ( lUpdated ) {					
						MsgInfo( 'Data updated'  )						
					} 					
